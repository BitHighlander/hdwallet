              ...
              <ul>
                <li :class="[ tabOpen === 'word1' ? 'is-active' : '']">
                  <a @click="tabOpen='word1' " class="is-large">
                    <input
                      style="border:none;border-style: none;width: 70px;"
...
import {version,initWallet} from './config'
...
export default {
  ...
  methods: {
    async createWallet(){
      ...
      const simpleCrypto = new SimpleCrypto(this.password);
      const hash = bcrypt.hashSync(this.password, 10);

      //encrypt seeds
      const encryptedString = simpleCrypto.encrypt(this.seeds);
